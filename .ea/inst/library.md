...
## optim library view
最后, 优化Library 页面: 在 "chapters" 模式下 (是不是换成 "units" 更直观?)
- 我看不到每个 Unit 的概况 (从 xxx 到 xxy), 只能 "盲选"
- 和 "words" 模式下的 Word list 检查UI 不一致, 可否复用呢? -- 默认在 Library 里面应该是看 words, 现在语义边界有点模糊? 进入 "Typing/flashcard" 练习模式可以作为两个按钮的入口, 而非现在两套交互模式
```sh
┌── Word Library ──────────────────────────────────────────┐
│ Book: GRE 3000 ▼   Unit: Unit 1 (aback - apparel) ▼      │
│ State: All ▼   Sort: Book Order ▼   [Search]             │
├──────────────────────────────────────────────────────────┤
│ ABCD..  │  aback           │  WordDetailView             │
│ ──────  │  abandon          │  (右侧 split 详情)            │
│ ──────  │  abase            │                             │
│ ──────  │  abdicate         │                             │
│ ──────  │  …                │                             │
│         │  apparel  (last)  │                             │
├─────────┴───────────────────┴─────────────────────────────┤
│ Showing 12 of 12 (Unit 1)  [▭ Flashcard]  [⌨ Typing]      │
└──────────────────────────────────────────────────────────┘
```
- 决策: 点 [Flashcard] / [Typing] 的时候, 直接开始学习
再 review 一下Library 页面的逻辑:
- state/sort 的逻辑和选 unit 的选词逻辑, 两者是否冲突?
  - 之前的4种 sort 模型是什么? 都有必要吗?
- 从 first principle 出发, 用户在这个页面中预期的行为是怎样的? 怎样操作最直观?
- 你上面的 ASCII UI 里面, 最左侧的 "ABCD.." 是什么意思?
```sh
# 用户在 Library 想干什么? 我列 6 种意图,标必要性:
┌───────────────────────────────┬──────┬────────────────────────────────────────────────────────────┐
│             意图              │ 频率 │                        通过什么完成                        │
├───────────────────────────────┼──────┼────────────────────────────────────────────────────────────┤
│ 1. 找一个特定词查看详情       │ 高   │ Search OR Alphabetical+Jump                                │
├───────────────────────────────┼──────┼────────────────────────────────────────────────────────────┤
│ 2. 浏览一本书                 │ 高   │ Book picker + Book Order sort + Unit picker                │
├───────────────────────────────┼──────┼────────────────────────────────────────────────────────────┤
│ 3. 看自己学到哪了             │ 中   │ Book picker + 进度感 (state filter or unit complete badge) │
├───────────────────────────────┼──────┼────────────────────────────────────────────────────────────┤
│ 4. 在某段范围里挑词练         │ 中   │ Book + Unit picker → Flashcard/Typing                      │
├───────────────────────────────┼──────┼────────────────────────────────────────────────────────────┤
│ 5. 看所有 New / Mastered 的词 │ 低   │ State filter                                               │
├───────────────────────────────┼──────┼────────────────────────────────────────────────────────────┤
│ 6. 找老错的词 / 快到期的词    │ 低   │ (Today recap / Plan 已经能做了)                            │
└───────────────────────────────┴──────┴────────────────────────────────────────────────────────────┘
```
总结来说, 是不是就两个场景 (选择一本书之后):
- book 级别: search; sort (book/a-z 2 种模式); A-Z jump (a-z sort 下); 
  - State filter 应该也是这个场景的? 后者 unit 模式下, 因为每个 unit 单词有限在每个单词后标记状态即可
  - 分页逻辑: Unit 模式下肯定不需要, 在 book view 下分析性能影响, 是否需要呢? 有个担心是 page & a-z jump 之间有所冲突?
- Unit 级别: 查看至今的进度
