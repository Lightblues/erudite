...
## final decision
还是有不符合约预期的地方! 我重新想了一下焦点模型:
- 快捷键应该简化为一个 (e.g. `cmd+.`, 可设置), 简化心智理解
  - 任意状态下按一次, 1. 若没有展开 chat 面板则展开; 2. 聚焦到输入框中;
  - 在焦点位于 chat 页面的时候, 再按一下, 则关闭 chat 面板
  - 当焦点在 chat 页面的时候, 任意的键盘输入都会出现在输入框中 (chat 页面应该不需要其他键盘操作)
- chat 页和主体 flashcard/typing 页的关系:
  - chat 应该是"陪伴工具", 即用即走,
  - 因此, cmd+. 进入 chat 状态;
  - 在 chat 状态 (解决了我的问题), 我也希望快速回到主体部分, 感觉 1. chat 页面关闭; 2. 定义快捷键 `esc` 退出 chat 页面焦点. 之后, 焦点应该重新回到主页
你看一下是否合理? 
```sh
# 这个模型非常合理，比之前的双快捷键更清晰。本质上是一个状态机：
┌─────────────┐  ⌘.   ┌─────────────┐  ⌘.   ┌─────────────┐
│ Main Focus  │ ────→  │ Chat Focus  │ ────→  │ Panel Close │
│ (panel可开可关)│ ←──── │ (panel open) │       │ → Main Focus│
└─────────────┘  Esc   └─────────────┘       └─────────────┘
```

好多了! 还有的问题是:
- 没有 chat 页面的时候, `cmd+.` 只是显示了对话但没聚焦到输入框无法输入?
- 我点击上面的对话历史的时候 (e.g. 选择 agent 输出的文本), 键盘输入就无法放到输入框中了! (因为没有聚焦在上面吗? 这是swift 的特点吗? 是否可解?)
```sh
┌────────────────────────┬────────────────────────────────────┐
│          操作          │                行为                │
├────────────────────────┼────────────────────────────────────┤
│ ⌘. / 点按钮 (panel 关) │ 打开 + 聚焦输入框                  │
├────────────────────────┼────────────────────────────────────┤
│ ⌘. / 点按钮 (panel 开) │ 关闭                               │
├────────────────────────┼────────────────────────────────────┤
│ Esc (焦点在输入框)     │ 取消聚焦，回到主内容，panel 保持开 │
├────────────────────────┼────────────────────────────────────┤
│ 点击输入框             │ 原生获焦（TextEditor 自带）        │
└────────────────────────┴────────────────────────────────────┘
```

1. 你的修改完全没生效, 算了, 原本也行? 是不是可以恢复一下?
2. 我发现点击右上角的图标也会有三个状态: 1. 弹出 chat 页面; 2. 焦点到 input; 3. 关闭 chat 页面. 
  - 逻辑很奇怪. 用户一般认为只有两个: 1. 点开直接可输入; 2. 再点一下关闭
  - 对于 "点选对话内容" (不再聚焦 inpu 栏) 的时候无法输入的问题, 先不管了. 我觉得基础体验优化一下即可

"toggle" 逻辑实现了, 但是开启 panel 之后仍然没有自动聚焦到输入框, 还得用鼠标点一下!

我们之前尝试修改了 Erudite 的交互模型:
- 在中间部分 flashcard/typing 操作的时候 (在其他 tab 的时候也一样, 反正都是转移到侧边栏), 通过 `cmd+.` 快捷键将焦点转移到右侧 chat 页面 (如果还没激活的话, 弹出 chat 页面) -- 快速可以打字和 agent 交流;
- 焦点在 chat 页面的时候, 通过 `esc` 快捷键将焦点重新转移到主页面中 -- 继续背单词/拼写练习
但改了很久, 还是有问题:
1. 当 chat 页面没弹出的时候按快捷键, 仍然没有做到自动聚焦到 input
2. 我点击了对话历史文本, 再点击 主体页面 (文字或者空白区域), 是希望重新回到学习状态, 但此时键盘按键都不生效了!

将交互模型整理成完整的 spec 文件保存起来. 参见 @.ea/spec/README.md . 此外, 针对之前的一些踩坑经验, 可否新整理个文件, 按照时间记录每一点?

更新 issues 系统, 内容参见 19/20 的 agent 实现 & memory 机制
